...
function compute_gradient!(∇ϕ::MVector{X, N, NS},
    ...
    if _residual_norm(Fz, opts) < opts.e_norm_tol
        return ∇ϕ, Fz
    end
...
function _e_norm_opt(fwd_cache, z::MVector{X, N, NS}, dz::MVector{X, N, NS}, λ::Real, Fz::MVector{X, N, NS}, opts) where {X, N, NS}
    # Evaluate F(z + λ*dz) without updating the base state representations (using standard evaluate if possible, otherwise we reuse update!)
    # But since update! mutates cache state which we need for J^T, we should be careful.
    # In line search, we evaluate 0.5 * ||F(z + λ*dz)||^2
    # To keep this clean, let's just step z temporarily
    z .+= λ .* dz
    update!(fwd_cache, Fz, z)
    val = norm(Fz)^2
    z .-= λ .* dz
...
function _search_lbfgs_opt!(Gs, Ls, S, D, z0, fwd_cache, opts)
    ...
    for iter = 1:opts.maxiter
        _residual_norm(opt_cache.Fz_curr, opts) < opts.e_norm_tol && break
